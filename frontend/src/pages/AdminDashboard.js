...
const AdminDashboard = () => {
	...
	const [selectedCourseForAssignment, setSelectedCourseForAssignment] =
		useState(null);
	...
	const [showLiveClassAttendanceModal, setShowLiveClassAttendanceModal] =
		useState(false);
	...
	const fetchesInProgress = useRef({
		courses: false,
		students: false,
		upcoming: false,
	});
	...
	const fetchCourses = useCallback(async () => {
		if (fetchesInProgress.current.courses) return;
		fetchesInProgress.current.courses = true;
		try {
			setLoading(true);
			...
			setLoading(false);
			fetchesInProgress.current.courses = false;
		}
	}, []);

	const fetchStudents = useCallback(async () => {
		if (fetchesInProgress.current.students) return;
		fetchesInProgress.current.students = true;
		try {
			...
			fetchesInProgress.current.students = false;
		}
	}, []);
	...
	const fetchUpcomingClasses = useCallback(async () => {
		if (fetchesInProgress.current.upcoming) return;
		fetchesInProgress.current.upcoming = true;
		try {
			const res = await courseAPI.getAllUpcomingLiveClasses();
			...
			fetchesInProgress.current.upcoming = false;
		}
	}, []);
	...
	useEffect(() => {
		console.log("AdminDashboard: Mount effect running (fetches)");
		fetchCourses();
		fetchStudents();
		fetchUpcomingClasses();
	}, [fetchCourses, fetchStudents, fetchUpcomingClasses]);
	...
	const handleAddCourse = async (e) => {
		...
		try {
			...
			if (
				...
				showMessage(
					"error",
					"Please fill in all numeric fields (Price, Duration, Max Students)",
				);
				return;
			}
		...
		if (result.isConfirmed) {
			try {
				...
				localStorage.setItem(
					"course_deleted",
					JSON.stringify({ courseId, timestamp: Date.now() }),
				);
				window.dispatchEvent(
					new CustomEvent("course_deleted", { detail: { courseId } }),
				);
	...
	const handleAddAssignment = async (e) => {
		...
		try {
			...
			if (
				...
				!assignmentForm.totalPoints
			) {
				showMessage(
					"error",
					"Please fill in all assignment fields (Title, Description, Due Date, Total Points)",
				);
				return;
			}
			...
			console.log(
				"Adding assignment for course:",
				selectedCourseForAssignment._id,
			);
			console.log("Assignment data:", assignmentData);

			await courseAPI.addAssignment(
				selectedCourseForAssignment._id,
				assignmentData,
			);
	...
	const handleMarkAttendance = async (e) => {
		e.preventDefault();
		try {
			await courseAPI.markAttendance(
				selectedCourseForAttendance._id,
				attendanceForm,
			);
		...
		<div style={{ maxWidth: "1400px", margin: "0 auto" }}>
			...
			<div
				...
				<div>
					...
					<p
						...
						Manage courses, assignments, and student engagement.
					</p>
				</div>
			</div>
			...
			<div
				...
				<div
					...
					{["courses", "assignments", "upcomingClasses", "attendance"].map(
						(tab) => (
							<button
								key={tab}
					...
					<div
						...
						<div
							...
							<p style={{ margin: "0 0 1rem 0", color: "#6b7280" }}>
								Course: {selectedCourseForAssignment.title}
							</p>
							...
							<form
								onSubmit={async (e) => {
									...
									try {
										if (
											!markForm.assignmentId ||
											!markForm.studentId ||
											markForm.marks === ""
										) {
											showMessage(
												"error",
												"Please select assignment, student and enter marks",
											);
											return;
										}
										await courseAPI.markAssignment(markForm.assignmentId, {
								...
								<div style={{ marginBottom: "1rem" }}>
									...
									<select
										required
										value={markForm.assignmentId}
										onChange={(e) =>
											setMarkForm({ ...markForm, assignmentId: e.target.value })
										}
										style={{
											width: "100%",
											padding: "0.8rem",
										...
										{assignmentsForCourse.map((a) => (
											<option key={a._id} value={a._id}>
												{a.title} — due {a.dueDate?.split("T")[0]}
											</option>
										))}
									</select>
								</div>
						...
						<div
							...
							<form
								onSubmit={async (e) => {
									e.preventDefault();
									try {
										if (!liveForm.title || !liveForm.scheduledAt) {
											showMessage(
												"error",
												"Please provide title and scheduled time",
											);
											return;
										}
										const payload = { ...liveForm };
										await courseAPI.addLiveClass(
											selectedCourseForAssignment._id,
											payload,
										);
					...
					<div
						...
						{upcomingClasses.filter(
							(cls) =>
								...
								String(cls.course_id || cls.course) ===
									String(selectedCourseForClass._id),
						).length === 0 ? (
							<p
							...
							<div style={{ display: "grid", gap: "1.5rem" }}>
								{upcomingClasses
									.filter(
										(cls) =>
											!selectedCourseForClass ||
											cls.course === selectedCourseForClass._id,
									)
									.map((liveClass) => (
										<div
											key={liveClass._id}
				...
				<div
					...
					<div
						...
						<div
							...
							{selectedLiveClass.courseName} •{" "}
							{new Date(selectedLiveClass.scheduledAt).toLocaleString()}
						</div>
								...
								{liveClassEnrollments.map((e) => (
									<div
										...
										<select
											value={
												selectedLiveClass &&
												liveClassAttendanceState[e.student._id]
													? liveClassAttendanceState[e.student._id]
													: "present"
											}
											onChange={(ev) => {
												const newState = { ...liveClassAttendanceState };
												newState[e.student._id] = ev.target.value;
												setLiveClassAttendanceState(newState);
											}}
											style={{
												padding: "0.5rem",
												border: "1px solid #ddd",
						...
						<div
							...
							<button
								onClick={async () => {
									const attendance = Object.keys(liveClassAttendanceState).map(
										(studentId) => ({
											studentId,
									...
									try {
										await courseAPI.markLiveClassAttendance(
						...
						<form
							...
							<div>
								...
								<textarea
									...
									onChange={(e) =>
										setLiveForm({ ...liveForm, description: e.target.value })
									}
