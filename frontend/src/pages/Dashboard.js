...
const Dashboard = () => {
				...
				enrollments.map(async (enr) => {
					try {
						const [detailRes, liveRes] = await Promise.all([
							...
							courseAPI
								.getCourseLiveClasses(enr.course._id)
								.catch(() => ({ data: [] })),
						]);
	...
	const handleRefreshEnrollment = async (enrollmentId) => {
		console.log("Refreshing enrollment:", enrollmentId);
		await fetchDashboardData();
	};

	// No changes needed to fetch logic as backend parity is handled
			...
			<div
				...
				<div>
					...
					<p
						...
						You have {stats.upcomingAssignments} upcoming assignments this week.
					</p>
				</div>
				<div style={{ display: "none" }}>
					{/* Placeholder for potential weather or quick info */}
				</div>
			</div>
				...
				<div
					...
					{[
						...
						},
						{
							...
							value: loading ? "..." : stats.upcomingAssignments,
						},
					].map((stat, idx) => (
						<div
							key={idx}
								...
								<div
									...
									<div style={{ padding: "1.5rem" }}>
										...
										<p
											...
											<strong>Duration:</strong> {enrollment.course?.duration}{" "}
											weeks
										</p>
										<div
											style={{
												...
												<div style={{ marginTop: "1rem" }}>
													...
													<p
														...
														{enrollment.course?.schedule?.days?.join(", ") ||
															"Schedule not set"}{" "}
														• {enrollment.course?.schedule?.startTime || "--"} -{" "}
														{enrollment.course?.schedule?.endTime || "--"}
													</p>
													...
													<div
														...
														<div>
															Attendance:{" "}
															{
																enrollment.detail.attendance
																	.attendancePercentage
															}
															%
														</div>
														<div style={{ marginTop: "0.5rem" }}>
															{enrollment.detail.attendance.records
																.slice(0, 3)
																.map((r) => (
																	<div
																		key={r._id}
														...
														{enrollment.liveClasses &&
															enrollment.liveClasses.length > 0 && (
